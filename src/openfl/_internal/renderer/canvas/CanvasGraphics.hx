...
class CanvasGraphics {
	...
	private static function createGradientPattern(type:GradientType, colors:Array<Dynamic>, alphas:Array<Dynamic>, ratios:Array<Dynamic>, matrix:Matrix,
			spreadMethod:SpreadMethod, interpolationMethod:InterpolationMethod, focalPointRatio:Float):CanvasGradient {
		...
		if (!stroke) {
			if (hasFill || bitmapFill != null) {
				...
				if (pendingMatrix != null) {
					...
					if (!hitTesting)
						context.fill(windingRule);
					context.transform(inversePendingMatrix.a, inversePendingMatrix.b, inversePendingMatrix.c, inversePendingMatrix.d, inversePendingMatrix.tx,
