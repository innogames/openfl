			...
			if (((textEngine.text == null || textEngine.text == "")
				...
				if ((textEngine.text != null && textEngine.text != "") || textEngine.__hasFocus) {
						...
						if (textField.__caretIndex > -1 && textEngine.selectable) {
							if (textField.__selectionIndex == textField.__caretIndex) {
								if (textField.__showCursor
									...
									drawCursor(context, group.offsetX
										+ advance
										- textField.scrollH, scrollY
										+ 2,
										TextEngine.getFormatHeight(textField.defaultTextFormat)
										- 3, group.format.color);
								}
					...
					if (textField.__caretIndex > -1 && textEngine.selectable && textField.__showCursor) {
						...
						drawCursor(context, scrollX + offsetX, scrollY + 2, TextEngine.getFormatHeight(textField.defaultTextFormat) - 3,
							textField.defaultTextFormat.color);
					}
				}
