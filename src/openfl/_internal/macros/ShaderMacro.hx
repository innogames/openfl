...
class ShaderMacro {
	...
	public static function build():Array<Field> {
		...
		if (glVertexSource != null || glFragmentSource != null) {
			...
			var isBaseClass = switch localClass {
				case {pack: ["openfl", "display"], name: "Shader"}: true;
				case _: false;
			};
		...
		while (regex.matchSub(source, lastMatch)) {
			...
			if (StringTools.startsWith(type, "sampler")) {
				fields.push({
					name: name,
					access: [APublic],
					kind: FVar(macro:openfl.display.ShaderParameter.ShaderParameterSampler),
					pos: pos
				});
			} else {
				var type = if (!isUniform) macro:openfl.display.ShaderParameter.ShaderParameterAttrib else switch (type) {
