...
class TextField extends InteractiveObject {
	...
	private function __setScrollHV(scrollH:Int, scrollV:Int) {
		...
		if (scrollH > __textEngine.maxScrollH)
			scrollH = __textEngine.maxScrollH;
		if (scrollH < 0)
			scrollH = 0;

		if (scrollV > __textEngine.maxScrollV)
			scrollV = __textEngine.maxScrollV;
		if (scrollV < 1)
			scrollV = 1;

		if (scrollH == __textEngine.scrollH && scrollV == __textEngine.scrollV)
			return;
