...
class TextField extends InteractiveObject implements IShaderDrawable {
	...
	private function __getCharIndexOnDifferentLine (charIndex:Int, lineIndex:Int):Int {
		...
		for (group in __textEngine.layoutGroups) {
			...
			if (charIndex >= group.startIndex && charIndex <= group.endIndex) {
				...
				if (y != null) return __getPosition (x, y, lineGroup);
			...
			if (group.lineIndex == lineIndex) {
				...
				if (x != null) return __getPosition (x, y, lineGroup);
	...
	private function set_autoSize (value:TextFieldAutoSize):TextFieldAutoSize {
		...
		if (value != __textEngine.autoSize) {
			...
			if (!__autoSizeDirty && !wordWrap && (value == RIGHT || value == CENTER)) {
				...
				var cachedWidth = __textEngine.width;
				__textEngine.update ();
				__autoSizeWidth = __textEngine.width;
				__textEngine.width = cachedWidth;
				
				switch (value) {
					
					case RIGHT: __autoSizeXOffset = cachedWidth - __autoSizeWidth;
					
					case CENTER: __autoSizeXOffset = (cachedWidth - __autoSizeWidth) * 0.5;
	...
	private function set_text (value:String):String {
		...
		if (!wordWrap && (autoSize == RIGHT || autoSize == CENTER)) {
			...
			switch (autoSize) {
				...
				case RIGHT: __autoSizeXOffset += __autoSizeWidth - __textEngine.width;
				
				case CENTER: __autoSizeXOffset += (__autoSizeWidth - __textEngine.width) * 0.5;
	...
	private function window_onKeyDown (key:KeyCode, modifier:KeyModifier):Void {
		...
		var currentCaretIndex = __caretIndex;
		var currentNumLines = __textEngine.numLines;
