...
final class Graphics {
	...
	public function lineStyle(thickness:Null<Float> = null, color:Int = 0, alpha:Float = 1, pixelHinting:Bool = false,
		...
		if (thickness != null) {
			if (joints == JointStyle.MITER) {
				if (thickness > __strokePadding)
					__strokePadding = thickness;
			} else {
				if (thickness / 2 > __strokePadding)
					__strokePadding = thickness / 2;
			}
		}
	...
	function __getBatchQuad(renderSession:RenderSession, alpha, colorTransform, blendMode):Quad {
		if (__batchQuadDirty) {
			...
			var transform = renderSession.renderer.getDisplayTransformTempMatrix(__worldTransform, false);
